...
inline void performIteration(tpe **&h_d_u, tpe **&h_d_uNew, const size_t nx, const size_t ny,
    ...
    for (auto py = 0; py < patch_ny; ++py) {
        for (auto px = 0; px < patch_nx; ++px) {
            ...
            if (px > 0)
                exchangeWest<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[patch]>>>(h_d_u[patch], h_d_u[py * patch_nx + px - 1], nx, ny);
            else
                applyBCWest<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[patch]>>>(h_d_u[patch], nx, ny);

            if (px < patch_nx - 1)
                exchangeEast<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[patch]>>>(h_d_u[patch], h_d_u[py * patch_nx + px + 1], nx, ny);
            else
                applyBCEast<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[patch]>>>(h_d_u[patch], nx, ny);

            if (py > 0)
                exchangeSouth<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[patch]>>>(h_d_u[patch], h_d_u[(py - 1) * patch_nx + px], nx, ny);
            else
                applyBCSouth<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[patch]>>>(h_d_u[patch], nx, ny);

            if (py < patch_ny - 1)
                exchangeNorth<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[patch]>>>(h_d_u[patch], h_d_u[(py + 1) * patch_nx + px], nx, ny);
            else
                applyBCNorth<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[patch]>>>(h_d_u[patch], nx, ny);
        }
    }
    ...
    for (auto py = 0; py < patch_ny; ++py) {
        for (auto px = 0; px < patch_nx; ++px) {
            ...
            stencil2d<<<numBlocks, blockSize, 0, streams[patch]>>>(h_d_u[patch], h_d_uNew[patch], nx, ny);
        }
    }
