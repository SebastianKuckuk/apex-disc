...
inline void performIteration(Patch<tpe> *patches, unsigned int numPatches, const size_t nx, const size_t ny,
    ...
    for (auto p = 0; p < numPatches; ++p) {
        ...
        if (REMOTE == patch.neighborType[0]) {
            MPI_Wait(&patch.reqsSend[0], MPI_STATUS_IGNORE);
        ...
        if (REMOTE == patch.neighborType[1]) {
            MPI_Wait(&patch.reqsSend[1], MPI_STATUS_IGNORE);
        ...
        if (REMOTE == patch.neighborType[2]) {
            MPI_Wait(&patch.reqsSend[2], MPI_STATUS_IGNORE);
        ...
        if (REMOTE == patch.neighborType[3]) {
            MPI_Wait(&patch.reqsSend[3], MPI_STATUS_IGNORE);
...
inline int realMain(int argc, char *argv[], MPI_Datatype MPI_TPE) {
    ...
    for (size_t i = 0; i < nItWarmUp; ++i)
        performIteration(patches, numPatches, nx, ny, mpi_rank, mpi_x, mpi_y, mpi_nx, mpi_ny, MPI_TPE);
    checkCudaError(cudaDeviceSynchronize(), true);
