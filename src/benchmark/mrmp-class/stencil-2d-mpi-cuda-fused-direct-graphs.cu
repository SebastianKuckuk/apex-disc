...
inline void performIteration(Patch<tpe> *patches, Patch<tpe> *d_patches, unsigned int numPatches, const size_t nx, const size_t ny,
    ...
    if (firstTime) {
        ...
        for (auto p = 0; p < numPatches; ++p) {
            checkCudaError(cudaStreamBeginCapture(streams[p], cudaStreamCaptureModeGlobal));

            stencil2d<<<numBlocks, blockSize, 0, streams[p]>>>(d_patches, p, nx, ny, numPatches);

            checkCudaError(cudaStreamEndCapture(streams[p], &graphs[p]));

            checkCudaError(cudaGraphInstantiate(&graphExecs[p], graphs[p], nullptr, nullptr, 0));
        }
    }

    for (auto p = 0; p < numPatches; ++p)
        checkCudaError(cudaGraphLaunch(graphExecs[p], streams[p]));
...
inline int realMain(int argc, char *argv[], MPI_Datatype MPI_TPE) {
    ...
    cudaGraph_t *graphs;
    checkCudaError(cudaMallocHost((void **)&graphs, sizeof(cudaGraph_t) * numPatches));
    cudaGraphExec_t *graphExecs;
    checkCudaError(cudaMallocHost((void **)&graphExecs, sizeof(cudaGraphExec_t) * numPatches));
    ...
    for (auto p = 0; p < numPatches; ++p) {
        checkCudaError(cudaGraphDestroy(graphs[p]));
        checkCudaError(cudaGraphExecDestroy(graphExecs[p]));
    }
