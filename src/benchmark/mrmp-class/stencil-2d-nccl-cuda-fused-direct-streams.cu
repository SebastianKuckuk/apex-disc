...
inline void performIteration(Patch<tpe> *patches, Patch<tpe> *d_patches, unsigned int numPatches, const size_t nx, const size_t ny,
                             ...
                             ncclComm_t *nccl_comms, cudaStream_t *streams) {
            ...
            for (auto i = 0; i < 4; ++i) {
                if (REMOTE == patch.neighborType[i]) {
                    checkNcclError(ncclSend(patch.h_d_bufSend[i], i < 2 ? ny - 2 : nx - 2, nccl_tpe,
                                   patch.neighborMpiRank[i], nccl_comms[p], streams[p]));
                    checkNcclError(ncclRecv(patch.h_d_bufRecv[i], i < 2 ? ny - 2 : nx - 2, nccl_tpe,
                                   patch.neighborMpiRank[i], nccl_comms[patch.neighborPatchIdx[i]], streams[p]));
                }
            }
        }
    }    
...
inline int realMain(int argc, char *argv[], MPI_Datatype MPI_TPE, ncclDataType_t nccl_tpe) {
    ...
    for (size_t i = 0; i < nItWarmUp; ++i)
        performIteration(patches, d_patches, numPatches, nx, ny, mpi_rank, mpi_x, mpi_y, mpi_nx, mpi_ny, nccl_tpe, nccl_comms, streams);
