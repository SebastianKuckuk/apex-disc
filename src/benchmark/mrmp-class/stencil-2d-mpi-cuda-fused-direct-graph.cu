...
template <typename tpe>
__global__ void stencil2d(Patch<tpe> *patches, const unsigned int p, const size_t nx, const size_t ny, unsigned int numPatches) {
...
inline void performIteration(Patch<tpe> *patches, Patch<tpe> *d_patches, unsigned int numPatches, const size_t nx, const size_t ny,
                             ...
                             cudaStream_t *streams, cudaGraph_t *graphs, cudaGraphExec_t *graphExecs) {
    ...
    if (firstTime) {
        ...
        for (auto p = 0; p < numPatches; ++p) {
            if (p > 0)
                checkCudaError(cudaStreamWaitEvent(streams[p], branchEvent, 0));
            stencil2d<<<numBlocks, blockSize, 0, streams[p]>>>(d_patches, p, nx, ny, numPatches);
            if (p > 0) {
                checkCudaError(cudaEventRecord(events[p], streams[p]));
                checkCudaError(cudaStreamWaitEvent(streams[0], events[p], 0));
            }
        }

        swapFields<<<1, numPatches, 0, streams[0]>>>(d_patches, numPatches);

        checkCudaError(cudaStreamEndCapture(streams[0], &graphs[0]));

        checkCudaError(cudaGraphInstantiate(&graphExecs[0], graphs[0], nullptr, nullptr, 0));

        for (auto p = 0; p < numPatches; ++p)
            checkCudaError(cudaEventDestroy(events[p]));
        checkCudaError(cudaFreeHost(events));

        checkCudaError(cudaEventDestroy(branchEvent));
    }

    checkCudaError(cudaGraphLaunch(graphExecs[0], streams[0]));
...
inline int realMain(int argc, char *argv[], MPI_Datatype MPI_TPE) {
    ...
    for (auto p = 0; p < numPatches; ++p)
        checkCudaError(cudaStreamCreate(&streams[p]));

    cudaGraph_t *graphs;
    checkCudaError(cudaMallocHost((void **)&graphs, sizeof(cudaGraph_t) * 1));
    cudaGraphExec_t *graphExecs;
    checkCudaError(cudaMallocHost((void **)&graphExecs, sizeof(cudaGraphExec_t) * 1));
    ...
    for (size_t i = 0; i < nItWarmUp; ++i)
        performIteration(patches, d_patches, numPatches, nx, ny, mpi_rank, mpi_x, mpi_y, mpi_nx, mpi_ny, MPI_TPE, streams, graphs, graphExecs);
    ...
    for (auto i = 0; i < 1; ++i) {
        checkCudaError(cudaGraphDestroy(graphs[i]));
        checkCudaError(cudaGraphExecDestroy(graphExecs[i]));
    }
    checkCudaError(cudaFreeHost(graphs));
    checkCudaError(cudaFreeHost(graphExecs));

    for (auto p = 0; p < numPatches; ++p)
        checkCudaError(cudaStreamDestroy(streams[p]));
