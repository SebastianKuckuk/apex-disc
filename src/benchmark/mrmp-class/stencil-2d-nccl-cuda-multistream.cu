...
inline void performIteration(Patch<tpe> *patches, unsigned int numPatches, const size_t nx, const size_t ny,
    ...
    checkNcclError(ncclGroupStart());
    {
        for (auto p = 0; p < numPatches; ++p) {
            ...
            if (BOUNDARY == patch.neighborType[0])
                applyBCWest<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 0]>>>(patch.d_u, nx, ny);
            else if (LOCAL == patch.neighborType[0])
                exchangeWest<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 0]>>>(patch.d_u, patches[patch.neighborPatchIdx[0]].d_u, nx, ny);
            else
                packBufferVertical<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 0]>>>(patch.d_u, patch.h_d_bufSend[0], nx + 1, nx, ny);

            if (BOUNDARY == patch.neighborType[1])            
                applyBCEast<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 1]>>>(patch.d_u, nx, ny);
            else if (LOCAL == patch.neighborType[1])
                exchangeEast<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 1]>>>(patch.d_u, patches[patch.neighborPatchIdx[1]].d_u, nx, ny);
            else
                packBufferVertical<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 1]>>>(patch.d_u, patch.h_d_bufSend[1], nx + nx - 2, nx, ny);

            if (BOUNDARY == patch.neighborType[2])
                applyBCSouth<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 2]>>>(patch.d_u, nx, ny);
            else if (LOCAL == patch.neighborType[2])
                exchangeSouth<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 2]>>>(patch.d_u, patches[patch.neighborPatchIdx[2]].d_u, nx, ny);
            else
                packBufferHorizontal<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 2]>>>(patch.d_u, patch.h_d_bufSend[2], nx + 1, nx, ny);

            if (BOUNDARY == patch.neighborType[3])
                applyBCNorth<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 3]>>>(patch.d_u, nx, ny);
            else if (LOCAL == patch.neighborType[3])
                exchangeNorth<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 3]>>>(patch.d_u, patches[patch.neighborPatchIdx[3]].d_u, nx, ny);
            else
                packBufferHorizontal<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 3]>>>(patch.d_u, patch.h_d_bufSend[3], (ny - 2) * nx + 1, nx, ny);

            for (auto i = 0; i < 4; ++i) {
                if (REMOTE == patch.neighborType[i]) {
                    checkNcclError(ncclSend(patch.h_d_bufSend[i], i < 2 ? ny - 2 : nx - 2, nccl_tpe,
                                   patch.neighborMpiRank[i], nccl_comms[p], streams[5 * p + i]));
                    checkNcclError(ncclRecv(patch.h_d_bufRecv[i], i < 2 ? ny - 2 : nx - 2, nccl_tpe,
                                   patch.neighborMpiRank[i], nccl_comms[patch.neighborPatchIdx[i]], streams[5 * p + i]));
                }
            }

            if (REMOTE == patch.neighborType[0])
                unpackBufferVertical<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 0]>>>(patch.h_d_bufRecv[0], patch.d_u, nx, nx, ny);
            if (REMOTE == patch.neighborType[1])
                unpackBufferVertical<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 1]>>>(patch.h_d_bufRecv[1], patch.d_u, nx + nx - 1, nx, ny);
            if (REMOTE == patch.neighborType[2])
                unpackBufferHorizontal<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 2]>>>(patch.h_d_bufRecv[2], patch.d_u, 1, nx, ny);
            if (REMOTE == patch.neighborType[3])
                unpackBufferHorizontal<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[5 * p + 3]>>>(patch.h_d_bufRecv[3], patch.d_u, (ny - 1) * nx + 1, nx, ny);
        }
    }
    ...
    for (auto p = 0; p < numPatches; ++p) {
        auto& patch = patches[p];
        stencil2d<<<numBlocks, blockSize, 0, streams[5 * p + 4]>>>(patch.d_u, patch.d_uNew, nx, ny);
    }
...
inline int realMain(int argc, char *argv[], MPI_Datatype MPI_TPE, ncclDataType_t nccl_tpe) {
    ...
    for (size_t i = 0; i < nItWarmUp; ++i)
        performIteration(patches, numPatches, nx, ny, mpi_rank, mpi_x, mpi_y, mpi_nx, mpi_ny, nccl_tpe, nccl_comms, streams);
    checkCudaError(cudaDeviceSynchronize(), true);
