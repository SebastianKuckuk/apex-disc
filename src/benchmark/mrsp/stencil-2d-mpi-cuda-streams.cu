...
inline void performIteration(tpe *&d_u, tpe *&d_uNew, tpe *h_d_bufSend[4], tpe *h_d_bufRecv[4], const size_t nx, const size_t ny,
    ...
    if (mpi_x > 0)
        packBufferVertical<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[0]>>>(d_u, h_d_bufSend[0], nx + 1, nx, ny);
    else
        applyBCWest<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[0]>>>(d_u, nx, ny);

    if (mpi_x < mpi_nx - 1)
        packBufferVertical<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[1]>>>(d_u, h_d_bufSend[1], nx + nx - 2, nx, ny);
    else
        applyBCEast<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[1]>>>(d_u, nx, ny);

    if (mpi_y > 0)
        packBufferHorizontal<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[2]>>>(d_u, h_d_bufSend[2], nx + 1, nx, ny);
    else
        applyBCSouth<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[2]>>>(d_u, nx, ny);

    if (mpi_y < mpi_ny - 1)
        packBufferHorizontal<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[3]>>>(d_u, h_d_bufSend[3], (ny - 2) * nx + 1, nx, ny);
    else
        applyBCNorth<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[3]>>>(d_u, nx, ny);
    ...
    if (mpi_x > 0) {
        ...
        unpackBufferVertical<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[0]>>>(h_d_bufRecv[0], d_u, nx, nx, ny);
    }
    if (mpi_x < mpi_nx - 1) {
        MPI_Wait(&reqs[5], MPI_STATUS_IGNORE);
        unpackBufferVertical<<<ceilingDivide(ny - 2, blockSize1D), blockSize1D, 0, streams[1]>>>(h_d_bufRecv[1], d_u, nx + nx - 1, nx, ny);
    }
    if (mpi_y > 0) {
        MPI_Wait(&reqs[6], MPI_STATUS_IGNORE);
        unpackBufferHorizontal<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[2]>>>(h_d_bufRecv[2], d_u, 1, nx, ny);
    }
    if (mpi_y < mpi_ny - 1) {
        MPI_Wait(&reqs[7], MPI_STATUS_IGNORE);
        unpackBufferHorizontal<<<ceilingDivide(nx - 2, blockSize1D), blockSize1D, 0, streams[3]>>>(h_d_bufRecv[3], d_u, (ny - 1) * nx + 1, nx, ny);
    }
